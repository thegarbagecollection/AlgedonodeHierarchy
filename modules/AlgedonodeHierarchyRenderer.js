...
class AlgedonodeHierarchyRenderer {
  ...
  strip(column, offset) {
    ...
    this.rectangle({lineWidth: 1.0, strokeStyle: this.colourHandler.strip, fillStyle: this.colourHandler.strip}, 
        stripTL.x, stripTL.y, this.columnWidth, height)
  }
  ...
  lightConnection(theLight, column, rowOffset, aOrB, active, activationSource) {
    ...
    this.line({lineWidth, strokeStyle}, 
        {x, y}, 
        {x:x, y:y + this.rowHeight / 4}, 
        {x:x + this.columnWidth / 4, y:y + this.rowHeight / 4 })
  ...
  elementCentre(row, column) {
    ...
    return {
      cX: columnOffset + column * this.columnSpacing,
      cY: rowOffset + row * this.rowSpacing,
    }
  }
